...
// customSpawnState holds tasks collected for a custom /go spawn.
...
func NewTelegramChannel(cfg *config.Config, bus *bus.MessageBus) (*TelegramChannel, error) {
	...
	return &TelegramChannel{
		...
		customSpawnMap: make(map[int64]*customSpawnState),
	}, nil
}

// SetAgentDeps injects agent capabilities for /status, /spawn, /model, etc.
func (c *TelegramChannel) SetAgentDeps(deps *TelegramAgentDeps) {
	c.agentDeps = deps
}
...
func (c *TelegramChannel) Start(ctx context.Context) error {
	...
	bh.HandleMessage(func(ctx *th.Context, message telego.Message) error {
		return c.handleStatus(ctx, &message)
	}, th.CommandEqual("status"))

	bh.HandleMessage(func(ctx *th.Context, message telego.Message) error {
		return c.handleSpawn(ctx, &message)
	}, th.CommandEqual("spawn"))

	bh.HandleMessage(func(ctx *th.Context, message telego.Message) error {
		return c.handleCustomSpawn(ctx, &message)
	}, th.CommandEqual("custom"))

	bh.HandleMessage(func(ctx *th.Context, message telego.Message) error {
		return c.handleGo(ctx, &message)
	}, th.CommandEqual("go"))
	...
	bh.HandleMessage(func(ctx *th.Context, message telego.Message) error {
		return c.handleModel(ctx, &message)
	}, th.CommandEqual("model"))

	bh.HandleCallbackQuery(c.handleCallbackQuery)
...
func (c *TelegramChannel) handleMessage(ctx context.Context, message *telego.Message) error {
	...
	if handled, tasks := c.getAndSetCustomTasks(chatID, content); handled {
		if len(tasks) == 0 {
			_ = c.sendPlain(ctx, chatID, "No tasks found. Send at least one task per line.")
			return nil
		}
