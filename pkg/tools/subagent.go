...
// ListTasksByChat returns tasks for the given channel and chat ID.
func (sm *SubagentManager) ListTasksByChat(originChannel, originChatID string) []*SubagentTask {
	sm.mu.RLock()
	defer sm.mu.RUnlock()

	tasks := make([]*SubagentTask, 0)
	for _, task := range sm.tasks {
		if task.OriginChannel == originChannel && task.OriginChatID == originChatID {
			tasks = append(tasks, task)
		}
	}
	return tasks
