...
func NewAgentRegistry(
	...
	if len(agentConfigs) == 0 {
		...
		memBackend := "filesystem"
		if cfg.Memory.Backend == "s3" && (cfg.Memory.S3.Bucket != "" || len(cfg.Memory.S3.Buckets) > 0) {
			memBackend = "s3"
		}
		logger.InfoCF("agent", "Created implicit main agent (no agents.list configured)", map[string]interface{}{"memory_backend": memBackend})
	} else {
